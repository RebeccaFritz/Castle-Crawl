...
(define POTATO (scale 3
                      (local [(define eye (circle 1 'solid 'white))]
...
(define (draw-keys levelkeys keys img)
  (cond [(empty? levelkeys) img]
        [else (local [(define keymatch (keyvkey (first levelkeys) keys))
                      (define jewel (circle 5 'solid (cond [(string=? (Levelkey-id (first levelkeys)) "A") 'white]
                                                           [(string=? (Levelkey-id (first levelkeys)) "B") 'red]
                                                           [(string=? (Levelkey-id (first levelkeys)) "C") 'orange]
                                                           [(string=? (Levelkey-id (first levelkeys)) "D") 'green]
                                                           [(string=? (Levelkey-id (first levelkeys)) "E") 'blue]
                                                           [(string=? (Levelkey-id (first levelkeys)) "F") 'pink]
                                                           )))]
                (cond [(boolean? keymatch) (draw-keys (rest levelkeys) keys img)]
                      [(not (Levelkey-have? (first levelkeys))) (draw-keys (rest levelkeys)
                                                                           keys
                                                                           (place-image
                                                                            (place-image jewel 20 15 BASEKEY)
                                                                            (Key-x keymatch)
                                                                            (Key-y keymatch)
                                                                            img))]
