...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname |Final Project - V9 - adding Key display|) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #t #t none #f ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp")) #f)))
(require racket/list)
...
(define HORIZONTAL-DOOR (rectangle 98 10 'solid 'lightbrown))
(define VERTICAL-DOOR (rectangle 10 98 'solid 'lightbrown))
; Doorknobs
...
(define VERTICAL-KNOB-F (rectangle 5 10 'solid 'pink))
(define HORIZONTAL-KNOB-F (rectangle 10 5 'solid 'pink))
...
(define PINKKEY (place-image (circle 5 'solid 'pink) 20 15 BASEKEY))                                                           
...
(define POTATO (scale 3 (local [(define eye (circle 1 'solid 'white))]
...
(define WALL (scale 0.57 (bitmap/file "wall-pixabay-nikiko-cropped.jpg")))
...
; turns a list of strings into an Item
...
(define (draw-keys-on-background levelkeys x img)
      (cond [(empty? levelkeys) img]
            ...
            [(string=? (Levelkey-id (first levelkeys)) "F")
             (draw-keys-on-background (rest levelkeys) (+ 50 x) (place-image PINKKEY x 50 img))]))
  ...
  (cond [(empty? levelkeys) img]
        [else (local [(define keymatch (keyvkey (first levelkeys) keys))
                      (define keyimage (cond [(string=? (Levelkey-id (first levelkeys)) "A") WHITEKEY]
                                          ...
                                          [(string=? (Levelkey-id (first levelkeys)) "F") PINKKEY]
                                          ))]
...
(define (render-Trollstate ws)
  (cond [(>= 0 (Trollstate-t ws)) (place-image MAN 200 460 WALL)]
        [(>= 20 (Trollstate-t ws)) (place-image (scale 1.2 TROLL) 200 460 WALL)]
        [(>= 40 (Trollstate-t ws)) (place-image (scale 1.4 TROLL) 200 460 WALL)]
        [(>= 60 (Trollstate-t ws)) (place-image (scale 1.6 TROLL) 200 460 WALL)]
        [(>= 80 (Trollstate-t ws)) (place-image (scale 1.8 TROLL) 200 460 WALL)]
        [else (place-image (scale 2 TROLL) 200 460 WALL)]))
...
; Player Structure, Room Structure -> Player Stucture
; Moves the player
...
(define testlevel3 (make-WS (make-Player (make-posn 350 350) "up/down" empty) room3.1 #|(map (lambda (li) (make-Levelkey (Levelkey-id li) #true)) level3keys)|# level3keys #false #false))
(define initialtrollstate (make-Trollstate 100))
