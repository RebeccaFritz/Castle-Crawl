...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname |Final Project - V2|) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #t #t none #f ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp")) #f)))
(require racket/list)
...
; door blue top #true #false 1.2 room -> [id pos locked? open? nxt# nxttype]
(define (lod->door los)
...
;[player lor]
(define initial-ws (make-WS (make-Player (make-posn 350 350) "up/down" empty) (first rooms)))
...
(define (render-WS ws)
  (place-image
   ...
   (draw-keys (Room-keys (WS-room ws))
...
(define (tock ws)
  (if (WS? ws)
      (local [(define newplayer (move-player (WS-player ws)))
        ...
        (if (WS? newroom) newroom (make-WS newplayer (WS-room ws))))
      ws))
...
; Worldstate -> Worldstate
; Changes which room is displayed if the door is open and the player is near the door
(define (change-room ws)
  (local [(define lod (Room-lod (WS-room ws)))
          ...
          (define newroom (foldr (lambda (room acc)
                                 ...
                                 (first rooms)
                                 rooms))
          (define newpage (foldr (lambda (page acc) (if (= (Door-nxt# (first lod))
                                                           (ExpoWS-page page)) page acc))
                                 (first pages)
                                 pages))]
    ; Is the player located near a door and is that door open
    (cond [(foldr (lambda (door acc) (and (string=? (player-position (WS-player ws)) (Door-pos door))
                                          (Door-open? door))) #false lod)
  ...
  (if (WS? ws)
      (make-WS (handle-movement (WS-player ws) key)
               (make-Room (Room-rid (WS-room ws))
                          (take-key (WS-player ws) (Room-keys (WS-room ws)) key)
...
(define (open-door ws lod key)
  (local [(define keys (Room-keys (WS-room ws)))
          (define player (WS-player ws))]
    (if (string=? key "e")
        (cond [(empty? keys) lod]
              [(empty? lod) lod]
...
(define (key-for-door keys door)
  (not (empty? (filter (lambda (key)
                         (and (string=? (Key-id key) (Door-id door))
                              (Key-have? key)))
                       keys))))
;(check-expect (key-for-door (list (make-Key "blue" 20 30 #true) (make-Key "yellow" 40 50 #false)) (make-Door "blue" "top" #true #false 1.2 room)) (make-Key "blue" 20 30 #true))
...
(define (next-page ws key)
  (local [(define nextpage (filter (lambda (li) (= (+ 0.1 (ExpoWS-page ws)) (ExpoWS-page li))) pages))
         (define nextroom (foldr (lambda (li acc) (if (= (+ 0.1 (floor (ExpoWS-page ws))) (Room-rid li)) li acc)) (first rooms) rooms))]
  (if (string=? key "\r")
      (if (empty? nextpage) (make-WS (WS-player initial-ws) nextroom) (first nextpage)) ws)))
