...
(define POTATO (place-image (circle 1 'solid 'white) 17 4
                            (place-image (circle 1 'solid 'white) 16 3
                                         (place-image (circle 1 'solid 'white) 12 9
                                                      (place-image (circle 1 'solid 'white) 5 8
                                                                   (place-image (circle 1 'solid 'white)  10 7 (ellipse 20 10 'solid 'brown)))))))
...
(define page3.1 (make-ExpoWS 3.1 "Finally you will retrieve the potato on the third floor."))
...
(define (tock ws)
  (if (WS? ws)
      (local [(define newplayer (move-player (WS-player ws)))
        ...
        (if (WS? newroom) newroom (make-WS newplayer (WS-room ws) (WS-levelkeys ws))))
      ws))
...
(define (take-key player levelkeys keys key)
  (local [(define keymatch (keyvkey (first levelkeys) keys))]
    (if (and (string=? "e" key)
             (not (boolean? keymatch)))
        (map (lambda (levelkey) (if (and (<= (- (Key-x keymatch) 50) (posn-x (Player-pos player)) (+ (Key-x keymatch) 50))
...
(define (open-door ws lod key)
  (local [(define keys (Room-keys (WS-room ws)))
    ...
    (if (string=? key "e")
        (cond [(empty? keys) lod]
              [(empty? lod) lod]
              [(and (key-for-door (WS-levelkeys ws) (first lod))
